<template>
  <div class="container">
    <Breadcrumb :items="['menu.bot', 'menu.bot.filterWords']" />
    <a-card class="general-card" :title="$t('menu.bot.filterWords')">
      <a-row>
        <a-col :flex="1">
          <a-form
            :model="formModel"
              ...
              <a-col :span="8">
                ...
                >
                  <a-input
                    ...
                    :placeholder="$t('searchTable.form.id')"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item field="word" :label="$t('keyWords.form.word')">
                  <a-input
                    v-model="formModel.word"
                    :placeholder="$t('keyWords.form.word.placeholder')"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item field="type" :label="$t('keyWords.form.type')">
                  <a-select
                    v-model="formModel.type"
                    :placeholder="$t('keyWords.form.type.placeholder')"
                    :options="typeOptions"
                    :allow-clear="true"
                  />
                </a-form-item>
      ...
      >
        <template #type="{ record }">
          {{ $t(`keyWords.form.type.${record.type}`) }}
        </template>
    ...
    <a-modal v-if="addVisible" v-model:visible="addVisible" title-align="start"
      ...
      <a-form ref="addFromRef" :model="addForm" @submit="handleSubmit">
        <a-form-item field="word" label="过滤词"
                     :rules="[{required:true,message:'必填'}]">
          <a-textarea v-model="addForm.word" placeholder="请输入过滤词" />
        </a-form-item>
        <a-form-item field="type" label="过滤类型"
                     :rules="[{required:true,message:'必填'}]">
          <a-select
    ...
    <a-modal v-if="updateVisible" v-model:visible="updateVisible" title-align="start"
      ...
      <a-form :model="updateForm" @submit="handleUpdateSubmit">
        ...
        <a-form-item field="word" label="过滤词"
                     :rules="[{required:true,message:'必填'}]">
          <a-textarea v-model="updateForm.word" placeholder="请输入过滤词" />
        </a-form-item>
        <a-form-item field="type" label="过滤类型"
                     :rules="[{required:true,message:'必填'}]">
          <a-select
...
import {
  addFilterWord,
  AddFilterWordsBody, del, disable, enable, FilterWordsParams,
  FilterWordsRecord, queryFilterWordsList,
  updateFilterWord,
  UpdateFilterWordsBody
} from "@/api/filter-words";
...
const { t } = useI18n();
const renderData = ref<FilterWordsRecord[]>([]);
...
const addForm = reactive<AddFilterWordsBody>({
  word: "",
  type: 1,
});
const updateForm = reactive<UpdateFilterWordsBody>({
  id: -1,
  word: "",
  type: -1,
});
...
const showUpdateKeyword = (record: FilterWordsRecord) => {
  updateForm.word = record.word;
...
const typeOptions = computed(() => [
  {
    label: t("keyWords.form.type.1"),
    value: 1
  },
  {
    label: t("keyWords.form.type.2"),
    value: 2
  }
]);
...
const columns = computed<TableColumnData[]>(() => [
  ...
  {
    title: t("keyWords.columns.word"),
    dataIndex: "word",
    slotName: "word",
    align: "center"
  },
  {
    title: t("keyWords.columns.type"),
    dataIndex: "type",
    slotName: "type",
    align: "center"
  },
...
const fetchData = async (
  params: FilterWordsParams = { current: 1, pageSize: 20 }
) => {
  setLoading(true);
  try {
    const { data } = await queryFilterWordsList(params);
...
const search = () => {
  ...
  fetchData({
    ...basePagination,
    ...formModel.value
  } as unknown as FilterWordsParams);
};
